"""HTML annotator for embedded JavaScript.

Extracts JavaScript/TypeScript from ``<script>`` blocks and delegates to the
existing TypeScript annotator. Non-script regions are masked rather than
removed so returned line numbers still point at the original HTML file.
"""
...
def annotate_html(source: str, source_name: str = "<html>") -> StructuralMetadata:
    lines = source.split("\n")
